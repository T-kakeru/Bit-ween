...
const LEGACY_TAB_TO_FILTER_ID: { [key: string]: any } = {
...
export const legacyTabToFilterId = (tab: string) => {
  return LEGACY_TAB_TO_FILTER_ID[tab] ?? DEFAULT_ARTICLE_FILTER_ID;
};

export const ARTICLE_FILTERS: { [key: string]: any } = {
  latest: {
    id: "latest",
    label: "最新",
    group: "並び替え",
    apply: (list: any[]) => sortByDateDesc(list),
  },
  thisWeek: {
    id: "thisWeek",
    label: "今週の記事",
  ...
  popular: {
    ...
    label: "人気",
    group: "フィルタ",
    apply: (list: any[]) => sortByDateDesc((list ?? []).filter((article) => article.isPopular)),
  },
  saved: {
    id: "saved",
    label: "保存済み",
    group: "フィルタ",
    apply: (list: any[]) => sortByDateDesc((list ?? []).filter((article) => article.isSaved)),
  },
  unread: {
    id: "unread",
    label: "未読",
    group: "フィルタ",
    apply: (list: any[]) => sortByDateDesc((list ?? []).filter((article) => !article.isRead)),
  },
  todayUnread: {
    id: "todayUnread",
    label: "今日の未読",
...
export const resolveArticleFilter = (filterId: any) => {
  return ARTICLE_FILTERS[filterId] ?? ARTICLE_FILTERS[DEFAULT_ARTICLE_FILTER_ID];
};

export const applyArticleFilter = (articles: any, filterId: any) => {
