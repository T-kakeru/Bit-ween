...
-- 目的: 大企業SES想定（15年運用）の分析用テストデータ投入
--
-- 使い方（超重要）
--  1) 下の "params" の値だけ編集
--  2) このSQLを実行
...
params AS (
  SELECT
    ...
    'テストカンパニー4'::text AS company_name,
    ...
    -- 規模
    1500::int AS active_count,
    8000::int AS retired_count,

    -- 期間（15年=180ヶ月）
    DATE '2011-04-01' AS base_month,
    180::int AS months_total,

    -- クライアント数（稼働者のみ付与）
    60::int AS client_count,

    -- 退職の波（% と 月範囲）
    -- 例: 2016-2019 を多め、2021-2022 をやや多め
    15::int AS retire_low1_pct,   -- 0..14
    55::int AS retire_peak_pct,   -- 15..54
    75::int AS retire_peak2_pct,  -- 55..74
    0::int AS retire_low1_start,
    24::int AS retire_low1_len,
    60::int AS retire_peak_start,
    48::int AS retire_peak_len,
    120::int AS retire_peak2_start,
    24::int AS retire_peak2_len,
    24::int AS retire_mid_start,
    96::int AS retire_mid_len,

    -- 勤続（月）
    6::int AS tenure_min_months,
    96::int AS tenure_span_months
),
upsert_company AS (
...
src_raw (
  ...
  -- ログインユーザー用（固定2名）
  SELECT
    'テスト管理者4', '男性', DATE '1985-04-01', p.admin_employee_code,
    '経営企画', DATE '2012-04-01', '稼働', NULL::text, NULL::date, NULL::text
  FROM params p

  UNION ALL
  SELECT
    'テスト閲覧者4', '女性', DATE '1988-10-10', p.viewer_employee_code,
    '経営企画', DATE '2015-04-01', '稼働', NULL::text, NULL::date, NULL::text
  FROM params p
  ...
  -- 在籍者
  SELECT
    '在籍社員' || LPAD(gs::text, 6, '0') AS full_name,
    CASE WHEN gs % 2 = 0 THEN '男性' ELSE '女性' END AS gender,
    (DATE '1975-01-01' + (gs * 13) * INTERVAL '1 day')::date AS birth_date,
    '46' || LPAD(gs::text, 6, '0') AS employee_code,
    CASE
      WHEN gs % 50 = 0 THEN '営業統括'
      WHEN gs % 50 IN (1, 2) THEN 'PMO'
      WHEN gs % 50 IN (3, 4, 5) THEN 'インフラ'
      WHEN gs % 50 IN (6, 7, 8) THEN 'クラウド'
      WHEN gs % 50 IN (9, 10, 11) THEN 'セキュリティ'
      WHEN gs % 50 IN (12, 13, 14, 15) THEN 'データ基盤'
      WHEN gs % 50 IN (16, 17, 18, 19, 20, 21, 22, 23, 24) THEN '第1開発'
      WHEN gs % 50 IN (25, 26, 27, 28, 29, 30, 31, 32, 33) THEN '第2開発'
      WHEN gs % 50 IN (34, 35, 36, 37, 38) THEN '第3開発'
      WHEN gs % 50 IN (39, 40, 41) THEN '運用保守'
      WHEN gs % 50 IN (42, 43) THEN '人事'
      ELSE '経営企画'
    END AS department_name,
    (p.base_month + (gs % p.months_total) * INTERVAL '1 month')::date AS join_date,
    CASE
      WHEN gs % 120 = 0 THEN '休職'
      WHEN gs % 18 = 0 THEN '待機'
      ELSE '稼働'
    END AS work_status_name,
    CASE
      WHEN (CASE WHEN gs % 120 = 0 THEN '休職' WHEN gs % 18 = 0 THEN '待機' ELSE '稼働' END) <> '稼働' THEN NULL
      ELSE 'エンタープライズクライアント' || LPAD((((gs % p.client_count) + 1))::text, 2, '0')
    END AS client_name,
  ...
  SELECT
    ...
    CASE
      ...
      WHEN EXTRACT(YEAR FROM age(t.retire_date, t.birth_date)) >= 40
        THEN CASE
          WHEN t.gs % 5 = 0 THEN '家庭問題'
          WHEN t.gs % 5 = 1 THEN '給与不満'
          WHEN t.gs % 5 = 2 THEN '会社不信'
          WHEN t.gs % 5 = 3 THEN '同業他社転職'
          ELSE 'ITモチベ低下'
        END
  ...
  FROM (
    SELECT
      gs,
      '退職社員' || LPAD(gs::text, 6, '0') AS full_name,
      CASE WHEN gs % 2 = 0 THEN '男性' ELSE '女性' END AS gender,
      (DATE '1970-01-01' + (gs * 7) * INTERVAL '1 day')::date AS birth_date,
      '46' || LPAD(gs::text, 6, '0') AS employee_code,
      CASE
        WHEN gs % 50 = 0 THEN '営業統括'
        WHEN gs % 50 IN (1, 2) THEN 'PMO'
      ...
      (p.base_month + (
        CASE
          WHEN (gs % 100) < p.retire_low1_pct THEN p.retire_low1_start + (gs % p.retire_low1_len)
          WHEN (gs % 100) < p.retire_peak_pct THEN p.retire_peak_start + (gs % p.retire_peak_len)
          WHEN (gs % 100) < p.retire_peak2_pct THEN p.retire_peak2_start + (gs % p.retire_peak2_len)
      ...
      CASE
        WHEN gs % 500 = 0 THEN '休職'
        WHEN gs % 120 = 0 THEN '待機'
        ELSE '稼働'
      END AS work_status_name,

      CASE
        WHEN (CASE WHEN gs % 500 = 0 THEN '休職' WHEN gs % 120 = 0 THEN '待機' ELSE '稼働' END) <> '稼働' THEN NULL
        ELSE 'エンタープライズクライアント' || LPAD((((gs % p.client_count) + 1))::text, 2, '0')
      END AS client_name
    FROM params p
